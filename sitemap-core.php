...
class GoogleSitemapGeneratorPath {
	...
	/**
	 * @var allow which kind of file to include
	 * @access private
	 */
	var $_fileType;
	

	function GoogleSitemapGeneratorPath($url="",$priority=0.0,$changeFreq="never",$fileType="*.*") {
		$this->SetUrl($url);
		$this->SetPriority($priority);
		$this->SetChangeFreq($changeFreq);
...
class GoogleSitemapGenerator {
	...
	*/
	...
	/**
	 * @var array The saved additional paths
	 */
	var $_paths = array();
	/**
	...
	*/
	...
	function LoadPaths() {
		global $wpdb;
        //echo "------------LoadPaths begin<br />";
		
		$needsUpdate=false;
		
		$pathsString=$wpdb->get_var("SELECT option_value FROM $wpdb->options WHERE option_name = 'sm_cpaths'");
		//echo "pathsString=".$pathsString."<br />";
        
		if(!empty($pathsString) && strpos($pathsString,"sm_path")!==false) {
			$pathsString = str_replace("O:7:\"sm_path\"","O:26:\"GoogleSitemapGeneratorPath\"",$pathsString);
            //echo "after str_replace, pathsString=".$pathsString."<br />";
			$needsUpdate=true;
		}
		
		if(!empty($pathsString)) {
			$storedpaths=unserialize($pathsString);
            //echo "not empty, storedpaths=".$storedpaths."<br />";
			$this->_paths=$storedpaths;
		} else {
			$this->_paths=array();
		}
		
        //echo "end, _paths=".$this->_paths."<br />";
        //echo "count(_paths)=".count($this->_paths)."<br />";
        
		if($needsUpdate) $this->SavePaths();
        //echo "------------LoadPaths end<br />";
	}
	...
	*/
	...
	function SavePaths() {
        //echo "--------SavePaths begin<br />";
        
		$oldvalue = get_option("sm_cpaths");
        //echo "oldvalue=".$oldvalue."<br />";
		if($oldvalue == $this->_paths) {
            //echo "paths is same<br />";
			return true;
		} else {
            //echo "SavePaths else part<br />";
            
			delete_option("sm_cpaths");
			//Add the option, Note the autoload=false because when the autoload happens, our class GoogleSitemapGeneratorPath doesn't exist
			add_option("sm_cpaths",$this->_paths,null,"no");
			return true;
		}
	}
	...
	*/
	...
	function HtmlApplyPaths() {
        ...
        //Array with all change frequencies
		$paths_cf=(!isset($_POST["sm_paths_cf"]) || !is_array($_POST["sm_paths_cf"])?array():$_POST["sm_paths_cf"]);
		
        //Array with all file types
		$paths_ft=(!isset($_POST["sm_paths_ft"]) || !is_array($_POST["sm_paths_ft"])?array():$_POST["sm_paths_ft"]);
		...
		if(isset($_POST["sm_paths_mark"]) && is_array($_POST["sm_paths_mark"])) {
			for($i=0; $i<count($_POST["sm_paths_mark"]); $i++) {
				...
				$p->SetUrl($paths_ur[$i]);
				$p->SetPriority($paths_pr[$i]);
                $p->SetChangeFreq($paths_cf[$i]);
                $p->SetFileType($paths_ft[$i]);
        ...
        //echo "HtmlApplyPaths: paths count=".count($paths)."<br />";

		return $paths;
	}
    
    function genPathsPages($paths) {
        $allPages = array();
        $pathNum = count($paths);
        //echo "pathNum=".$pathNum."<br />";
        
        for($i=0; $i<$pathNum; $i++) {
            //Create new object
            $singlePathPages = $this->processSinglePath($paths[$i]);

            //Add it to the array
            $allPages = array_merge($allPages, $singlePathPages);
        }
        
        return $allPages;
    }
    
	function genSinglePage($loc, $priority, $changefreq, $lastmod) {
        //Create new object
        $singlePage=new GoogleSitemapGeneratorPage();
        
        $singlePage->SetUrl($loc);
        $singlePage->SetPriority($priority);
        $singlePage->SetChangeFreq($changefreq);
        $singlePage->SetLastMod($lastmod);
        
        //array_push($pages,$singlePage);
        
        //echo "Url=".$singlePage->GetUrl()."<br />";
        //echo "Priority=".$singlePage->GetPriority()."<br />";
        //echo "ChangeFreq=".$singlePage->GetChangeFreq()."<br />";
        //echo "LastMod=".$singlePage->GetLastMod()."<br />";
    ...
    //each is a GoogleSitemapGeneratorPath
    function processSinglePath($curPath)
    {
        //echo "now in processSinglePath"."<br />";
        $pages=array();
        
        $fullUrl = $curPath->GetUrl();
        //echo "input url=".$fullUrl."<br />";
        ...
        if($fullUrl=="" || (stripos($fullUrl, $homePath) < 0))
        {
            //echo "Error: input url=".$fullUrl." is null or not contain home page=".$homePath."<br />";
            return $pages;
        }
        
        $relativePath = str_replace($homePath, "", $fullUrl);
        //echo "input relativePath=".$relativePath."<br />";
        $relativePath = $this->addTailSlash($relativePath);
        //files/doc/docbook/
        //echo "input relativePath=".$relativePath."<br />";
        
        //echo phpinfo();
        //$rootPath = $_SERVER['DOCUMENT_ROOT'];//获得服务器文档根变量

        $rootPath = $this->GetHomePath();
        //echo "rootPath=".$rootPath."<br />";
        $rootPath = str_replace("\\", "/", $rootPath);
        //echo "rootPath=".$rootPath."<br />";
        $rootPath = $this->addTailSlash($rootPath);
        //echo "rootPath=".$rootPath."<br />";
        ...
        //echo $fullPath." isDir=".$isDir."<br />";
        
        if(!$isDir)
        {
            echo $fullPath." is NOT valid directory !<br />";
            echo "Error: input url=".$fullUrl." is invalid !<br />";
            return $pages;
        }
        
        //echo $fullPath." is valid directory <br />";
        ...
        $urlPref = $homePath.$relativePath;

        //tmp set these fixed values
        $curPriority = $curPath->GetPriority();
        //$curChangeFreq = "daily";
        $curChangeFreq = $curPath->GetChangeFreq();
        $fileType = $curPath->GetFileType();
        
        //echo "curPriority=$curPriority<br />";
        //echo "curChangeFreq=$curChangeFreq<br />";
        $pages = $this->getPagesUnderPath($fullPath, $urlPref, $curPriority, $curChangeFreq, $fileType);
    ...
    function getPagesUnderPath($fullPath, $urlPref, $priority, $changefreq, $fileType)
    {
        //echo "now in getPagesUnderPath <br />";
        
        $fullPath = $this->addTailSlash($fullPath);
        $urlPref = $this->addTailSlash($urlPref);

        //echo "fullPath=".$fullPath."<br />";
        //echo "urlPref=".$urlPref."<br />";
        ...
        //$validSuf = array("html", "htm", "txt", "pdf", "php", "js", "cs", "py", "xml", "xsl", "mak", "mk", "asp", "sh", "bat", "cmd", "css", "diff", "patch", "c", "cpp", "h", "hpp", "asm");
        ...
        while(($filename = readdir($dirHandler)) !== false)
        {
            ...
            {
                //echo "omit filename=$filename <br />";
                continue;
            }
            ...
            $isDir = is_dir($fullFilename);
            //echo $fullFilename." is Dir = ".$isDir."<br />";
            
            if(!$isDir)
            {
                $fileSuf = $this->getFileSuffix($filename);
                //echo "fileSuf=".$fileSuf."<br />";
                                
                $fileValid = $this->strExistInArr($fileSuf, $validSuf);
                //echo "fileValid=".$fileValid."<br />";
                
                if($fileValid)
                {
                    //echo "valid suffix=".$fileSuf."<br />";
                    
                    $encodedFilename = rawurlencode($filename);
                    //echo "encodedFilename=".$encodedFilename."<br />";
                    $encodedFullUrl = $urlPref.$encodedFilename;
                    //echo "encodedFullUrl=".$encodedFullUrl."<br />";
                    
                    //echo "filetype=".filetype($fullFilename)."<br />";
                    
                    $lastModTime = filemtime($fullFilename);
                    //echo "lastModTime=".$lastModTime."<br />";
                    $changeTime = filectime($fullFilename);
                    //echo "changeTime=".$changeTime."<br />";
                    //use the latest time as last modified time
                    if($changeTime > $lastModTime)
                    {
                        $lastModTime = $changeTime;
                    }
                    
                    //echo "--------------------------------------<br />";
                    //echo "-------add singe page begin--------<br />";

                    $singlePage = $this->genSinglePage($encodedFullUrl, $priority, $changefreq, $lastModTime);
                    array_push($pages, $singlePage);
                    //echo "add single page OK, url=".$encodedFullUrl."<br />";

                    //echo "-------add singe page done--------<br />";
                }
                else
                {
                    //echo "Invalid file suffix for file=".$filename."<br />";
                }
            }
            else
            {
                $newFullPath = $this->addTailSlash($fullFilename);
                //echo "newFullPath=".$newFullPath."<br />";
                $newUrlPref = $urlPref.$filename."/";
                $subPages = $this->getPagesUnderPath($newFullPath, $newUrlPref, $priority, $changefreq, $fileType);
    ...
    function mergePathPages($pages, $pathsPages)
    {
        //echo "now in mergePathPages";
        
        for($i=0; $i<count($pathsPages); $i++)
        {
            $newPage = $pathsPages[$i];
            $newUrl = $newPage->GetUrl();
            $found = False;
            for($j = 0; $j < count($pages); $j++)
            {
                $oldPage = $pages[$j];
                if($oldPage->GetUrl() == $newUrl)
                {
                    //update it
                    $oldPage->SetPriority($newPage->GetPriority());
                    $oldPage->SetChangeFreq($newPage->GetChangeFreq());
                    $oldPage->SetLastMod($newPage->GetLastMod());
