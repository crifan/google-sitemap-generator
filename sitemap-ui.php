...
class GoogleSitemapGeneratorUI {
	...
	function HtmlShowOptionsPage() {
		...
		if(!empty($_REQUEST["sm_rebuild"])) { //Pressed Button: Rebuild Sitemap
            ...
            //$purePages  = $this->sg->HtmlApplyPages();;
			$pathsPages = $this->sg->genPathsPages($this->sg->_paths);
            $this->sg->_pages=$this->sg->mergePathPages($this->sg->_pages, $pathsPages);
			...
			if($this->sg->SavePaths()) $message.=__("Paths saved",'sitemap') . "<br />";
			else $message.=__('Error while saving paths', 'sitemap'). "<br />";
					...
					<?php $this->HtmlPrintBoxHeader('sm_paths',__('Additional paths', 'sitemap')); ?>
							...
							var paths = [ <?php
								if(count($this->sg->_paths)>0) {
									for($i=0; $i<count($this->sg->_paths); $i++) {
										$v=&$this->sg->_paths[$i];
										if($i>0) echo ",";
										echo '{url:"' . $v->getUrl() . '", priority:' . number_format($v->getPriority(),1,".","") . ', changeFreq:"' . $v->getChangeFreq() . '", fileType:"' . $v->getFileType() . '"}';
									}
								}
							?> ];
							//]]>
						...
						<table width="100%" cellpadding="3" cellspacing="3" id="sm_pathTable">
							<tr>
								...
								<th scope="col"><?php _e('Priority','sitemap'); ?></th>
								<th scope="col"><?php _e('Change Frequency','sitemap'); ?></th>
								<th scope="col"><?php _e('Included File Type','sitemap'); ?></th>
								<th scope="col"><?php _e('#','sitemap'); ?></th>
							</tr>
							<?php
								if(count($this->sg->_paths)<=0) { ?>
									<tr>
										<td colspan="5" align="center"><?php _e('No paths defined.','sitemap') ?></td>
									</tr><?php
								}
							?>
						</table>
