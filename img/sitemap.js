...
function sm_addPath(url,priority,changeFreq, fileType) {
	...
	var td = ce('TD');
	var iFiletype = ce('INPUT');
	iFiletype.type="text";
	iFiletype.style.width='95%';
	iFiletype.name="sm_paths_ft[]";
	if(fileType) iFiletype.value=fileType;
	td.appendChild(iFiletype);
	tr.appendChild(td);
...
function sm_loadPaths() {
	for(var i=0; i<paths.length; i++) {
		sm_addPath(paths[i].url,paths[i].priority,paths[i].changeFreq,paths[i].fileType);
	}
}